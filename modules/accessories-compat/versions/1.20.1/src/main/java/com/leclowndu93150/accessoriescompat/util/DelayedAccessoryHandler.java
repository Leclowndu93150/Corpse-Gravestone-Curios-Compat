...
public class DelayedAccessoryHandler {
    ...
    private static boolean tryEquip(ItemStack stack, AccessoriesCapability cap) {
        ...
        if (container != null && data.slotIndex() >= 0 && data.slotIndex() < container.getSize()) {
            if (container.getAccessories().getItem(data.slotIndex()).isEmpty()) {
                ItemStack clean = stack.copy();
                AccessorySlotDataComponent.removeSlotData(clean);

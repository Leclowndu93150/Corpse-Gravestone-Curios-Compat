...
public class Config {
    ...
    static {
        ...
        BLACKLISTED_ITEMS = COMMON_BUILDER
                .comment("Items that should not be transferred to accessory slots (format: 'modid:item')")
                .defineList("blacklisted_items", List.of(), entry -> entry instanceof String);

        TRANSFER_CURSED_ITEMS = COMMON_BUILDER
                .comment("Whether cursed items (Curse of Binding) should be transferred back to accessory slots")
